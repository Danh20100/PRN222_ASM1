        ...
        public static async Task Main(string[] args)
        {
            ...
            using (var scope = app.Services.CreateScope())
            {
                ...
                else
                {
                    ...
                    if (!db.ChunkingStrategies.Any())
                    {
                        db.ChunkingStrategies.AddRange(
                            new ChunkingStrategy { StrategyName = "Fixed-Size" },
                            new ChunkingStrategy { StrategyName = "Paragraph" },
                            new ChunkingStrategy { StrategyName = "Sentence" },
                            new ChunkingStrategy { StrategyName = "Recursive" }
                        );
                        await db.SaveChangesAsync();
                        Console.WriteLine("[SEED] Seeded Chunking Strategies.");
                    }

                    // Seed Embedding Models
                    if (!db.EmbeddingModels.Any())
                    {
                        db.EmbeddingModels.AddRange(
                            new EmbeddingModel { ModelName = "gemini-embedding-001", Provider = "Google" },
                            new EmbeddingModel { ModelName = "multilingual-e5-base", Provider = "HuggingFace" },
                            new EmbeddingModel { ModelName = "text-embedding-3-small", Provider = "OpenAI" },
                            new EmbeddingModel { ModelName = "PhoBERT-base", Provider = "VinAI" }
                        );
                        await db.SaveChangesAsync();
                        Console.WriteLine("[SEED] Seeded Embedding Models.");
                    }
